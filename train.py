...
output_dir = basepath + "Output/" + cond + "/Training/"
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(test_loader):
            ...
            if log_interval is not None and batch_idx % log_interval == 0:
                print("{} Test: [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(time.ctime(time.time()), batch_idx * len(data), len(test_loader.dataset), 100. * batch_idx / len(test_loader), loss.item()))
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, epochs + 1):
        train_loss = train(model, device, train_loader, optimizer, epoch, display_step)
